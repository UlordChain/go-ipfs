...
import (
	...
	fstest "gx/udfs/QmSJBsmLP1XMjv8hxYg2rUMdPDB7YUpyBo9idjrJ6Cmq6F/fuse/fs/fstestutil"
...
func randObj(t *testing.T, nd *core.UdfsNode, size int64) (ipld.Node, []byte) {
	buf := make([]byte, size)
...
func setupUdfsTest(t *testing.T, node *core.UdfsNode) (*core.UdfsNode, *fstest.Mount) {
...
func TestUdfsBasicRead(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	nd, mnt := setupUdfsTest(t, nil)
	defer mnt.Close()
...
func getPaths(t *testing.T, udfs *core.UdfsNode, name string, n *dag.ProtoNode) []string {
	if len(n.Links()) == 0 {
		return []string{name}
	}
	var out []string
	for _, lnk := range n.Links() {
		child, err := lnk.GetNode(udfs.Context(), udfs.DAG)
		if err != nil {
			t.Fatal(err)
		}
		...
		sub := getPaths(t, udfs, path.Join(name, lnk.Name), childpb)
		out = append(out, sub...)
	}
	return out
}
...
func TestUdfsStressRead(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	nd, mnt := setupUdfsTest(t, nil)
...
func TestUdfsBasicDirRead(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	nd, mnt := setupUdfsTest(t, nil)
