...
var (
	debug  = flag.Bool("debug", false, "direct udfs output to console")
...
func benchmarkAdd(amount int64) (*testing.BenchmarkResult, error) {
	...
	results := testing.Benchmark(func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			initCmd := exec.Command("udfs", "init", "-b=1024")
			setupCmd(initCmd)
			...
			func() {
				...
				if *online {
					daemonCmd := exec.Command("udfs", "daemon")
					setupCmd(daemonCmd)
				...
				b.StartTimer()
				addCmd := exec.Command("udfs", "add", f.Name())
				setupCmd(addCmd)
