...
test_dht() {
  ...
  # udfs dht findpeer <peerID>
  test_expect_success 'findpeer' '
    udfsi 1 dht findpeer $PEERID_0 | sort >actual &&
    udfsi 0 id -f "<addrs>" | cut -d / -f 1-5 | sort >expected &&
    test_cmp actual expected
  '
  
  # udfs dht put <key> <value>
  test_expect_success 'put with good keys' '
    udfsi 0 dht put "$TEST_DHT_PATH" "$TEST_DHT_VALUE" | sort >putted &&
    [ -s putted ] ||
    test_fsh cat putted
  '
  ...
  test_expect_success 'get with good keys' '
    HASH="$(echo "hello world" | udfsi 2 add -q)" &&
    udfsi 2 name publish "/udfs/$HASH" &&
    udfsi 1 dht get "/ipns/$PEERID_2" | grep -aq "/udfs/$HASH"
  '
  
  test_expect_success 'put with bad keys fails (issue #5113)' '
    udfsi 0 dht put "foo" "bar" >putted
    udfsi 0 dht put "/pk/foo" "bar" >>putted
    udfsi 0 dht put "/ipns/foo" "bar" >>putted
    [ ! -s putted ] ||
    test_fsh cat putted
  '
  
  test_expect_failure 'put with bad keys returns error (issue #4611)' '
    ! udfsi 0 dht put "foo" "bar" &&
    ! udfsi 0 dht put "/pk/foo" "bar" &&
    ! udfsi 0 dht put "/ipns/foo" "bar"
  '
  
  test_expect_failure 'get with bad keys (issue #4611)' '
    ! udfsi 0 dht get "foo" &&
    ! udfsi 0 dht get "/pk/foo"
  '
  
  test_expect_success "add a ref so we can find providers for it" '
    echo "some stuff" > afile &&
    HASH=$(udfsi 3 add -q afile)
  '
  
  # udfs dht findprovs <key>
  test_expect_success 'findprovs' '
    udfsi 4 dht findprovs $HASH > provs &&
    iptb get id 3 > expected &&
    test_cmp provs expected
  '
  
  
  # udfs dht query <peerID>
  ...
  # TODO: flaky. tracked by https://github.com/udfs/go-udfs/issues/2620
  test_expect_success 'query' '
    udfsi 3 dht query "$(echo banana | udfsi 3 add -q)" >actual &&
    udfsi 3 dht query "$(echo apple | udfsi 3 add -q)" >>actual &&
    udfsi 3 dht query "$(echo pear | udfsi 3 add -q)"  >>actual &&
