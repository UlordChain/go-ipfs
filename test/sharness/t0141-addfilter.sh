...
test_swarm_filter_cmd() {
  ...
  test_expect_success "'udfs swarm filters' succeeds" '
    udfs swarm filters > list_actual
  '

  test_expect_success "'udfs swarm filters' output looks good" '
...
test_config_swarm_addrfilters_cmd() {
  ...
  test_expect_success "'udfs config Swarm.AddrFilters' succeeds" '
    udfs config Swarm.AddrFilters > list_actual
  '
  ...
  test_expect_success "'udfs config Swarm.AddrFilters' output looks good" '
...
test_swarm_filters() {
  ...
  udfs swarm filters rm all

  test_swarm_filter_cmd

  test_config_swarm_addrfilters_cmd

  test_expect_success "'udfs swarm filter add' succeeds" '
    udfs swarm filters add $AF1 $AF2 $AF3
  '
  ...
  test_expect_success "'udfs swarm filter rm' succeeds" '
    udfs swarm filters rm $AF2 $AF3
  '
  ...
  test_expect_success "'udfs swarm filter add' succeeds" '
    udfs swarm filters add $AF4 $AF2
  '
  ...
  test_expect_success "'udfs swarm filter rm' succeeds" '
    udfs swarm filters rm $AF1 $AF2 $AF4
  '
...
test_expect_success "init without any filters" '
  echo "null" >expected &&
  udfs config Swarm.AddrFilters >actual &&
  test_cmp expected actual
'

test_expect_success "adding addresses to the config to filter succeeds" '
  udfs config --json Swarm.AddrFilters "[\"$AF1\", \"$AF4\"]"
'
