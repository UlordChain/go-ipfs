...
test_description="Test udfs repo pinning"
...
test_pin_flag() {
  ...
  if udfs pin ls --type="$ptype" "$object" >actual
  then
...
# https://github.com/udfs/go-udfs/issues/1075
...
test_expect_success "'udfs add dir' succeeds" '
  mkdir dir1 &&
  mkdir dir1/dir2 &&
  ...
  udfs add -q -r dir1 >actualall &&
  tail -n1 actualall >actual1 &&
  echo "$HASH_DIR1" >expected1 &&
  udfs repo gc && # remove the patch chaff
  test_cmp expected1 actual1
'

test_expect_success "objects are there" '
  udfs cat "$HASH_FILE6" >FILE6_a &&
  udfs cat "$HASH_FILE5" >FILE5_a &&
  udfs cat "$HASH_FILE4" >FILE4_a &&
  udfs cat "$HASH_FILE3" >FILE3_a &&
  udfs cat "$HASH_FILE2" >FILE2_a &&
  udfs cat "$HASH_FILE1" >FILE1_a &&
  udfs ls "$HASH_DIR3"   >DIR3_a &&
  udfs ls "$HASH_DIR4"   >DIR4_a &&
  udfs ls "$HASH_DIR2"   >DIR2_a &&
  udfs ls "$HASH_DIR1"   >DIR1_a
'

# saving this output for later
test_expect_success "udfs object links $HASH_DIR1 works" '
  udfs object links $HASH_DIR1 > DIR1_objlink
'
...
test_expect_success "nothing is pinned directly" '
  udfs pin ls --type=direct >actual4 &&
...
test_expect_success "objects are still there" '
  ...
  udfs cat "$HASH_FILE6"  >actual45 &&
  udfs cat "$HASH_FILE5" >>actual45 &&
  udfs cat "$HASH_FILE4" >>actual45 &&
  udfs cat "$HASH_FILE3" >>actual45 &&
  udfs cat "$HASH_FILE2" >>actual45 &&
  udfs cat "$HASH_FILE1" >>actual45 &&
  udfs ls "$HASH_DIR3"   >>actual45 &&
  udfs ls "$HASH_DIR4"   >>actual45 &&
  udfs ls "$HASH_DIR2"   >>actual45 &&
  udfs ls "$HASH_DIR1"   >>actual45 &&
  test_cmp expected45 actual45
'

test_expect_success "remove dir recursive pin succeeds" '
  echo "unpinned $HASH_DIR1" >expected5 &&
  udfs pin rm -r "$HASH_DIR1" >actual5 &&
...
test_expect_success "pin some directly and indirectly" '
  udfs pin add -r=false  "$HASH_DIR1"  >actual7 &&
  udfs pin add -r=true  "$HASH_DIR2"  >>actual7 &&
  udfs pin add -r=false  "$HASH_FILE1" >>actual7 &&
...
test_expect_success "some objects are still there" '
  ...
  udfs cat "$HASH_FILE6"  >actual8 &&
  udfs cat "$HASH_FILE4" >>actual8 &&
  udfs cat "$HASH_FILE2" >>actual8 &&
  udfs cat "$HASH_FILE1" >>actual8 &&
  udfs ls "$HASH_DIR4"   >>actual8 &&
  udfs ls "$HASH_DIR2"   >>actual8 &&
  udfs object links "$HASH_DIR1" >>actual8 &&
  test_cmp expected8 actual8
'

# todo: make this faster somehow.
test_expect_success "some are no longer there" '
  test_must_fail udfs cat "$HASH_FILE5" &&
  test_must_fail udfs cat "$HASH_FILE3" &&
  test_must_fail udfs ls "$HASH_DIR3"
'

test_expect_success "recursive pin fails without objects" '
  udfs pin rm -r=false "$HASH_DIR1" &&
  test_must_fail udfs pin add -r "$HASH_DIR1" 2>err_expected8 &&
...
test_expect_success "test add nopin file" '
  ...
  NOPINHASH=$(udfs add -q --pin=false test_nopin_data) &&
...
test_expect_success "test add nopin dir" '
  ...
  udfs add -q -r --pin=false nopin_dir1 | tail -n1 >actual1 &&
...
test_expect_success "test unpinning a hash that's not pinned" "
  test_expect_code 1 udfs pin rm $FICTIONAL_HASH --timeout=2s
  test_expect_code 1 udfs pin rm $FICTIONAL_HASH/a --timeout=2s
  test_expect_code 1 udfs pin rm $FICTIONAL_HASH/a/b --timeout=2s
"
