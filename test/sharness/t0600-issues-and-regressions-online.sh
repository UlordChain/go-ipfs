...
test_expect_success "udfs refs local over HTTP API returns NDJOSN not flat - #2803" '
  echo "Hello World" | udfs add &&
...
test_expect_success "metrics work" '
  ...
  grep "udfs_bs_cache_arc_hits_total" < pro_data ||
...
test_expect_success "udfs daemon --offline --mount fails - #2995" '
  test_expect_code 1 udfs daemon --offline --mount 2>daemon_err &&
