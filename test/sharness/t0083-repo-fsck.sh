...
test_description="Test udfs repo fsck operations"
...
test_expect_success "'udfs repo fsck' succeeds with no daemon running empty
repo.lock" '
  mkdir -p $UDFS_PATH &&
  mkdir -p $UDFS_PATH/datastore &&
  touch $UDFS_PATH/datastore/LOCK &&
  touch $UDFS_PATH/repo.lock &&
  printf "/ip4/127.0.0.1/tcp/5001" > "$UDFS_PATH/api" &&
  udfs repo fsck > fsck_out_actual1
'
test_expect_success "'udfs repo fsck' output looks good with no daemon" '
  grep "Lockfiles have been removed." fsck_out_actual1
'

# Make sure the files are actually removed
test_expect_success "'udfs repo fsck' confirm file deletion" '
  test ! -e "$UDFS_PATH/repo.lock" &&
  test ! -e "$UDFS_PATH/datastore/LOCK" &&
  test ! -e "$UDFS_PATH/api"
'

# Try with all lock files present: repo.lock, api, and datastore/LOCK with
# repo.lock is non-zero TODO: this test is broken until we find consensus on the
# non-zero repo.lock issue
test_expect_success "'udfs repo fsck' succeeds with no daemon running non-zero
repo.lock" '
  mkdir -p "$UDFS_PATH" &&
  printf ":D" > "$UDFS_PATH/repo.lock" &&
  touch "$UDFS_PATH/datastore/LOCK" &&
  udfs repo fsck > fsck_out_actual1b
'
test_expect_success "'udfs repo fsck' output looks good with no daemon" '
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  printf  "/ip4/127.0.0.1/tcp/5001" > "$UDFS_PATH/api" &&
  touch $UDFS_PATH/datastore/LOCK &&
  udfs repo fsck > fsck_out_actual2
'
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  printf  "/ip4/127.0.0.1/tcp/5001" > "$UDFS_PATH/api" &&
  touch $UDFS_PATH/repo.lock &&
  udfs repo fsck > fsck_out_actual3
'
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  touch $UDFS_PATH/repo.lock &&
  touch $UDFS_PATH/datastore/LOCK &&
  udfs repo fsck > fsck_out_actual4
'
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  touch $UDFS_PATH/repo.lock &&
  udfs repo fsck > fsck_out_actual5
'
test_expect_success "'udfs repo fsck' output looks good with no daemon" '
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  touch $UDFS_PATH/datastore/LOCK &&
  udfs repo fsck > fsck_out_actual6
'
test_expect_success "'udfs repo fsck' output looks good with no daemon" '
...
test_expect_success "'udfs repo fsck' succeeds partial lock" '
  printf "/ip4/127.0.0.1/tcp/5001" > "$UDFS_PATH/api" &&
  udfs repo fsck > fsck_out_actual7
'
...
test_expect_success "'udfs repo fsck' fails with daemon running" '
  ! (udfs repo fsck 2>fsck_out_actual8 )

'

test_expect_success "'udfs repo fsck' output looks good with daemon" '
  grep "Error: udfs daemon is running" fsck_out_actual8
'
