...
test_add_symlinks() {
  test_expect_success "udfs add files succeeds" '
    udfs add -q -r files >filehash_all &&
  ...
  test_expect_success "udfs add --cid-version=1 files succeeds" '
    udfs add -q -r --cid-version=1 files >filehash_all &&
    tail -n 1 filehash_all >filehash_out
  '
  ...
  test_expect_success "adding a symlink adds the link itself" '
    udfs add -q files/bar/baz > goodlink_out
  '
  ...
  test_expect_success "adding a broken symlink works" '
    udfs add -q files/bad > badlink_out
  '
...
adding with no symlink" '
    ...
    ln -s b files2/a/d
    udfs add -rq files2/a/b/c > no_sym &&
    udfs add -rq files2/a/d/c > sym &&
    test_cmp no_sym sym
  '
}
