#!/usr/bin/env bash
#
# Copyright (c) 2014 Christian Couder
# MIT Licensed; see the LICENSE file in this repository.
#

test_description="Test directory sharding"

. lib/test-lib.sh

test_expect_success "set up test data" '
  mkdir testdata
  for i in `seq 2000`
  do
    echo $i > testdata/file$i
  done
'

test_add_large_dir() {
  exphash="$1"
  test_expect_success "udfs add on very large directory succeeds" '
    udfs add -r -q testdata | tail -n1 > sharddir_out &&
    echo "$exphash" > sharddir_exp &&
    test_cmp sharddir_exp sharddir_out
  '
  test_expect_success "udfs get on very large directory succeeds" '
    udfs get -o testdata-out "$exphash" &&
    test_cmp testdata testdata-out
  '
}

test_init_udfs

UNSHARDED="QmavrTrQG4VhoJmantURAYuw3bowq3E2WcvP36NRQDAC1N"
test_add_large_dir "$UNSHARDED"

test_launch_udfs_daemon

test_add_large_dir "$UNSHARDED"

test_kill_udfs_daemon

test_expect_success "enable sharding" '
  udfs config --json Experimental.ShardingEnabled true
'

SHARDED="QmSCJD1KYLhVVHqBK3YyXuoEqHt7vggyJhzoFYbT8v1XYL"
test_add_large_dir "$SHARDED"

test_launch_udfs_daemon

test_add_large_dir "$SHARDED"

test_kill_udfs_daemon

test_expect_success "sharded and unsharded output look the same" '
  udfs ls "$SHARDED" | sort > sharded_out &&
  udfs ls "$UNSHARDED" | sort > unsharded_out &&
  test_cmp sharded_out unsharded_out
'

test_expect_success "udfs cat error output the same" '
  test_expect_code 1 udfs cat "$SHARDED" 2> sharded_err &&
  test_expect_code 1 udfs cat "$UNSHARDED" 2> unsharded_err &&
  test_cmp sharded_err unsharded_err
'

test_expect_success "'udfs ls --resolve-type=false' admits missing block" '
  udfs ls "$SHARDED" | head -1 > first_file &&
  read -r HASH _ NAME <first_file &&
  udfs pin rm "$SHARDED" "$UNSHARDED" && # To allow us to remove the block
  udfs block rm "$HASH" &&
  test_expect_code 1 udfs cat "$SHARDED/$NAME" &&
  test_expect_code 1 udfs ls "$SHARDED" &&
  udfs ls --resolve-type=false "$SHARDED" | sort > missing_out &&
  test_cmp sharded_out missing_out
'

test_launch_udfs_daemon

test_expect_success "gateway can resolve sharded dirs" '
  echo 100 > expected &&
  curl -sfo actual "http://127.0.0.1:$GWAY_PORT/udfs/$SHARDED/file100" &&
  test_cmp expected actual
'

test_kill_udfs_daemon

test_add_large_dir_v1() {
  exphash="$1"
  test_expect_success "udfs add (CIDv1) on very large directory succeeds" '
    udfs add -r -q --cid-version=1 testdata | tail -n1 > sharddir_out &&
    echo "$exphash" > sharddir_exp &&
    test_cmp sharddir_exp sharddir_out
  '

  test_expect_success "can access a path under the dir" '
    udfs cat "$exphash/file20" > file20_out &&
    test_cmp testdata/file20 file20_out
  '
}

# this hash implies the directory is CIDv1 and leaf entries are CIDv1 and raw
SHARDEDV1="zdj7WY8aNcxF49q1ZpFXfchNmbswnUxiVDVjmrHb53xRM8W4C"
test_add_large_dir_v1 "$SHARDEDV1"

test_launch_udfs_daemon

test_add_large_dir_v1 "$SHARDEDV1"

test_kill_udfs_daemon

test_done
