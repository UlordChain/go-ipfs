...
test_expect_success "Gateway on API unavailable" '
  HASH=$(echo "testing" | udfs add -q)
  test_curl_resp_http_code "http://127.0.0.1:$API_PORT/udfs/$HASH" "HTTP/1.1 404 Not Found"
'
test_kill_udfs_daemon

test_launch_udfs_daemon --unrestricted-api
test_expect_success "Gateway on --unrestricted-api API available" '
  HASH=$(echo "testing" | udfs add -q)
  test_curl_resp_http_code "http://127.0.0.1:$API_PORT/udfs/$HASH" "HTTP/1.1 200 OK"
'
test_kill_udfs_daemon
