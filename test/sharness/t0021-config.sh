...
test_config_cmd_set() {
  ...
  # flags (like --bool in "udfs config --bool")
...
# (i.e. just setting 'udfs config --json foo "[1, 2, 3]"') may
...
test_profile_apply_revert() {
  ...
  test_expect_success "save expected config" '
    udfs config show >expected
  '

  test_expect_success "'udfs config profile apply ${profile}' works" '
    udfs config profile apply '${profile}'
  '

  test_expect_success "profile ${profile} changed something" '
    udfs config show >actual &&
    test_must_fail test_cmp expected actual
  '

  test_expect_success "'udfs config profile apply ${inverse_profile}' works" '
    udfs config profile apply '${inverse_profile}'
  '
...
test_config_cmd() {
  ...
  test_expect_success "'udfs config show' works" '
    udfs config show >actual
  '

  test_expect_success "'udfs config show' output looks good" '
  ...
  test_expect_success "setup for config replace test" '
    cp "$UDFS_PATH/config" newconfig.json &&
    sed -i"~" -e /PrivKey/d -e s/10GB/11GB/ newconfig.json &&
    sed -i"~" -e '"'"'/PeerID/ {'"'"' -e '"'"' s/,$// '"'"' -e '"'"' } '"'"' newconfig.json
  '

  test_expect_success "run 'udfs config replace'" '
  udfs config replace - < newconfig.json
  '

  test_expect_success "check resulting config after 'udfs config replace'" '
    sed -e /PrivKey/d "$UDFS_PATH/config" > replconfig.json &&
  ...
  test_expect_success "'udfs config Identity' fails" '
    test_expect_code 1 udfs config Identity 2> ident_out
  '
  ...
  test_expect_success "'udfs config Identity.PrivKey' fails" '
    test_expect_code 1 udfs config Identity.PrivKey 2> ident_out
  '
  ...
  test_expect_success "lower cased PrivKey" '
    sed -i"~" -e '\''s/PrivKey/privkey/'\'' "$UDFS_PATH/config" &&
    test_expect_code 1 udfs config Identity.privkey 2> ident_out
  '
  ...
  test_expect_success "'udfs config show' doesn't include privkey" '
    udfs config show > show_config &&
    test_expect_code 1 grep PrivKey show_config
  '

  test_expect_success "'udfs config replace' injects privkey back" '
    udfs config replace show_config &&
    grep "\"PrivKey\":" "$UDFS_PATH/config" | grep -e ": \".\+\"" >/dev/null
  '

  test_expect_success "'udfs config replace' with privkey errors out" '
    cp "$UDFS_PATH/config" real_config &&
    test_expect_code 1 udfs config replace - < real_config 2> replace_out
  '
  ...
  test_expect_success "'udfs config replace' with lower case privkey errors out" '
    cp "$UDFS_PATH/config" real_config &&
  ...
  test_expect_success "copy udfs config" '
    cp "$UDFS_PATH/config" before_patch
  '

  test_expect_success "'udfs config profile apply server' works" '
    udfs config profile apply server
  '

  test_expect_success "backup was created and looks good" '
    test_cmp "$(find "$UDFS_PATH" -name "config-*")" before_patch
  '

  test_expect_success "'udfs config Swarm.AddrFilters' looks good with server profile" '
    udfs config Swarm.AddrFilters > actual_config &&
    test $(cat actual_config | wc -l) = 17
  '

  test_expect_success "'udfs config profile apply local-discovery' works" '
    udfs config profile apply local-discovery
  '

  test_expect_success "'udfs config Swarm.AddrFilters' looks good with applied local-discovery profile" '
    udfs config Swarm.AddrFilters > actual_config &&
    test $(cat actual_config | wc -l) = 1
  '
  ...
  test_expect_success "ensure test profile is applied fully" '
    udfs config profile apply test
  '
  ...
  # won't work as it changes datastore definition, which makes udfs not launch
  # without converting first
  # test_profile_apply_revert badgerds

  test_expect_success "cleanup config backups" '
    find "$UDFS_PATH" -name "config-*" -exec rm {} \;
  '
}
