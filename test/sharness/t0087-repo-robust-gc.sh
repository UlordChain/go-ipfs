...
test_gc_robust_part1() {
  ...
  test_expect_success "add a 1MB file with --raw-leaves" '
    ...
    HASH1=`udfs add --raw-leaves -q afile`
  '

  HASH1FILE=.udfs/blocks/L3/CIQNIPL4GP62ZMNNSLZ2G33Z3T5VAN3YHCJTGT5FG45XWH5FGZRXL3A.data

  LEAF1=zb2rhZWKCMcHCfCym41ocwE5BhNQsEVLrwBkLEQaxJzRHHZch
  LEAF1FILE=.udfs/blocks/C2/AFKREIBKRCW7HF6NHR6DVWECQXC5RQC7U7PKHKTI53BYYZNQP23DK5FC2Y.data

  LEAF2=zb2rhdUpCuyoQPRtvBW8JhAEfPaNd9dBb3QQCMMWYS1V8hjBR
  LEAF2FILE=.udfs/blocks/Q6/BAFKREIDFSUIR43GJPHNDXXQA45GJVNRZBET3CRPUMYJCBLK3RTN7ZAMQ6Q
  ...
  test_expect_success "check that the node is removed" '
    test_must_fail udfs cat $HASH1
  '

  test_expect_success "'udfs repo gc' should still be be fine" '
    udfs repo gc
  '
  ...
  test_expect_success "'udfs repo gc' should abort without removing anything" '
    test_must_fail udfs repo gc 2>&1 | tee gc_err &&
  ...
  test_expect_success "leaf nodes were not removed after gc" '
    udfs cat $LEAF3 > /dev/null &&
    udfs cat $LEAF4 > /dev/null
  '
  ...
  test_expect_success "create a permission problem" '
    ...
    test_must_fail udfs block rm $LEAF2 2>&1 | tee block_rm_err &&
    grep -q "permission denied" block_rm_err
  '

  test_expect_success "'udfs repo gc' should still run and remove as much as possible" '
    test_must_fail udfs repo gc 2>&1 | tee repo_gc_out &&
  ...
  test_expect_success "'udfs repo gc' should be ok now" '
    udfs repo gc | tee repo_gc_out
...
test_gc_robust_part2() {
  ...
  test_expect_success "add 1MB file normally (i.e., without raw leaves)" '
    random 1048576 56 > afile &&
    HASH2=`udfs add -q afile`
  '

  LEAF1=QmSijovevteoY63Uj1uC5b8pkpDU5Jgyk2dYBqz3sMJUPc
  LEAF1FILE=.udfs/blocks/ME/CIQECF2K344QITW5S6E6H6T4DOXDDB2XA2V7BBOCIMN2VVF4Q77SMEY.data

  LEAF2=QmTbPEyrA1JyGUHFvmtx1FNZVzdBreMv8Hc8jV9sBRWhNA
  LEAF2FILE=.udfs/blocks/WM/CIQE4EFIJN2SUTQYSKMKNG7VM75W3SXT6LWJCHJJ73UAWN73WCX3WMY.data

  test_expect_success "add some additional unpinned content" '
    random 1000 3 > junk1 &&
    random 1000 4 > junk2 &&
    JUNK1=`udfs add --pin=false -q junk1` &&
    JUNK2=`udfs add --pin=false -q junk2`
  '
  ...
  test_expect_success "'udfs repo gc' should abort" '
    test_must_fail udfs repo gc 2>&1 | tee repo_gc_out &&
  ...
  test_expect_success "test that garbage collector really aborted" '
    udfs cat $JUNK1 > /dev/null &&
    udfs cat $JUNK2 > /dev/null 
  '
  ...
  test_expect_success "'udfs repo gc' should abort with two errors" '
    test_must_fail udfs repo gc 2>&1 | tee repo_gc_out &&
    grep -q "could not retrieve links for $LEAF1" repo_gc_out &&
  ...
  test_expect_success "'udfs repo gc --stream-errors' should abort and report each error separately" '
    test_must_fail udfs repo gc --stream-errors 2>&1 | tee repo_gc_out &&
  ...
  test_expect_success "'udfs repo gc' should be fine now" '
    udfs repo gc | tee repo_gc_out &&
