...
test_add_skip() {
  ...
  test_expect_success "'udfs add -r' with hidden file succeeds" '
    ...
    udfs add -r mountdir/planets >actual
  '

  test_expect_success "'udfs add -r' did not include . files" '
    cat >expected <<-\EOF &&
...
EOF
  ...
  test_expect_success "'udfs add -r --hidden' succeeds" '
    udfs add -r --hidden mountdir/planets >actual
  '

  test_expect_success "'udfs add -r --hidden' did include . files" '
    cat >expected <<-\EOF &&
...
EOF
  ...
  test_expect_success "'udfs add' includes hidden files given explicitly even without --hidden" '
...
EOF
    udfs add mountdir/dotfiles/.vimrc >actual
    cat actual
    test_cmp expected actual
  '
