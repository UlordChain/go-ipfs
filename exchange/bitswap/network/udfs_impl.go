...
// NewFromUdfsHost returns a BitSwapNetwork supported by underlying UDFS host
func NewFromUdfsHost(host host.Host, r routing.ContentRouting) BitSwapNetwork {
	bitswapNetwork := impl{
		host:    host,
		routing: r,
	}
...
// impl transforms the udfs network interface, which sends and receives
