// Package path contains utilities to work with udfs paths.
package path
...
var (
	...
	ErrBadPath = errors.New("invalid 'udfs ref' path")
...
// A Path represents an udfs content path:
//   * /<cid>/path/to/file
//   * /udfs/<cid>
...
// IsJustAKey returns true if the path is of the form <key> or /udfs/<key>, or
// /ipld/<key>
func (p Path) IsJustAKey() bool {
	parts := p.Segments()
	return len(parts) == 2 && (parts[0] == "udfs" || parts[0] == "ipld")
}
...
// ParsePath returns a well-formed udfs Path.
// The returned path will always be prefixed with /udfs/ or /ipns/.
// The prefix will be added if not present in the given string.
// This function will return an error when the given string is
// not a valid udfs path.
...
// ParseCidToPath takes a CID in string form and returns a valid udfs Path.
...
// IsValid checks if a path is a valid udfs Path.
