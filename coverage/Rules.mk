...
# unit tests coverage
UTESTS_$(d) := $(shell go list -f '{{if (len .TestGoFiles)}}{{.ImportPath}}{{end}}' $(go-flags-with-tags) ./... | grep -v go-udfs/vendor | grep -v go-udfs/Godeps)
...
ifneq ($(filter coverage%,$(MAKECMDGOALS)),)
	...
	DEPS_test/sharness += cmd/udfs/udfs-test-cover $(d)/coverage_deps $(d)/udfs
endif

export UDFS_COVER_DIR:= $(realpath $(d))/sharnesscover/

$(d)/sharness_tests.coverprofile: export TEST_NO_PLUGIN=1
$(d)/sharness_tests.coverprofile: $(d)/udfs cmd/udfs/udfs-test-cover $(d)/coverage_deps test_sharness_short
