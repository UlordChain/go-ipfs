...
func main() {
	coverDir := os.Getenv("UDFS_COVER_DIR")
	if len(coverDir) == 0 {
		fmt.Println("UDFS_COVER_DIR not defined")
		os.Exit(1)
	}
	...
	p := exec.Command("udfs-test-cover", args...)
	p.Stdin = os.Stdin
	p.Stdout = os.Stdout
	p.Stderr = os.Stderr
	p.Env = append(os.Environ(), "UDFS_COVER_RET_FILE="+retFile.Name())
