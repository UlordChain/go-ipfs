...
func (rt *RoutingTable) Write(w io.Writer) {
	fmt.Fprintf(w, "Routing Table, bs = %d, Max latency = %d\n", rt.bucketsize, rt.maxLatency)
	rt.tabLock.RLock()
	...
	for i, b := range rt.Buckets {
		...
		for e := b.list.Front(); e != nil; e = e.Next() {
			...
			fmt.Fprintf(w, "\t\t- %s - %d %s  \n", p.Pretty(), commonPrefixLen(rt.local, ConvertPeerID(p)), rt.metrics.LatencyEWMA(p).String())
		}
		b.lk.RUnlock()
	}
	rt.tabLock.RUnlock()
