...
func (dht *IpfsDHT) findMasterPeerSingle(ctx context.Context, p peer.ID, id peer.ID) (*pb.Message, error) {
	...
	pmes := pb.NewMessage(pb.Message_FIND_MASTER_NODE, []byte(id), 0)
	resp, err := dht.sendRequest(ctx, p, pmes)
	switch err {
	case nil:
		return resp, nil
...
func (dht *IpfsDHT) nearestMasterPeersToQuery(pmes *pb.Message, count int) []peer.ID {
	closer := dht.routingTable.NearestMasterPeers(kb.ConvertKey(string(pmes.GetKey())), count)
	return closer
}
...
func (dht *IpfsDHT) betterMasterPeersToQuery(pmes *pb.Message, p peer.ID, count int) []peer.ID {
	...
	// no node? nil
	if closer == nil {
		log.Warning("betterMasterPeersToQuery: no closer peers to send:", p)
		return nil
	}
	...
	for _, clp := range closer {
		...
		// == to self? thats bad
		if clp == dht.self {
			log.Error("BUG betterMasterPeersToQuery: attempted to return self! this shouldn't happen...")
			return nil
		}
		// Dont send a peer back themselves
		if clp == p {
