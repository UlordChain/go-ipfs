...
func (dht *IpfsDHT) handlerForMsgType(t pb.Message_MessageType) dhtHandler {
	...
	case pb.Message_FIND_MASTER_NODE:
		return dht.handleFindMasterPeer
...
func (dht *IpfsDHT) handleFindMasterPeer(ctx context.Context, p peer.ID, pmes *pb.Message) (*pb.Message, error) {
	...
	if targetPid == dht.self {
		...
		closest = dht.betterMasterPeersToQuery(pmes, p, CloserPeerCount)
	...
	if closest == nil {
		log.Infof("%s handleFindMasterPeer %s: could not find anything.", dht.self, p)
		return resp, nil
	}
