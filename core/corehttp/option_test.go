...
func TestCheckVersionOption(t *testing.T) {
	tcs := []testcasecheckversion{
		{"/go-udfs/0.1/", APIPath + "/test/", false, "", http.StatusBadRequest},
		{"/go-udfs/0.1/", APIPath + "/version", true, "check!", http.StatusOK},
		{config.ApiVersion, APIPath + "/test", true, "check!", http.StatusOK},
		{"Mozilla Firefox/no go-udfs node", APIPath + "/test", true, "check!", http.StatusOK},
		{"/go-udfs/0.1/", "/webui", true, "check!", http.StatusOK},
	}
